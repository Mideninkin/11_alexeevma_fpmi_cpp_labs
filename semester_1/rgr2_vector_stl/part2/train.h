...
inline std::istream& operator>>(std::istream& instr, Train& train) {
    ...
    if (instr >> train.id_ >> type_str) {
        ...
        try {
            ...
            std::cerr << "Error in reading departure time: " << e.what() << std::endl;
            instr.setstate(std::ios::failbit);
            return instr;
        }
        
        if (!(instr >> hours >> delim2 >> minutes)) {
            instr.setstate(std::ios::failbit);
            return instr;
        }
        if (delim2 != ':') {
        ...
        try {
            ...
            std::cerr << "Error in reading travel time: " << e.what() << std::endl;
            instr.setstate(std::ios::failbit);
            return instr;
        }
    }
...
inline std::ostream& operator<<(std::ostream& outstr, const Train& train) {
    std::tm dep_tm = *std::localtime(&train.dispatch_time_);
